...
また、本システムは OpenAI API を業務ロジックの一部として利用する前提です。  
Discord上の会話内容を、そのまま保存するだけでなく、OpenAI API を通じて要約・タスク抽出・決定事項抽出・文面生成を行い、再利用しやすい形へ整理します。
...
- 将来的に他ツールを追加しやすい
- OpenAI API を利用して、会話内容の整理や文面生成を自動化できる
...
3. タスクや決定事項を保存できる  
4. 結果をPDFで出力できる  
5. OpenAI API を使った整理処理の土台を用意できる
...
- **業務ハブAPI / Pythonアプリ**: コマンド処理、要約処理、外部連携の中核
- **OpenAI API**: 要約、タスク抽出、決定事項抽出、文面生成を担当
...
- Discordを主操作画面とする
- 中央ハブでデータを一元管理する
- OpenAI API を会話整理・生成処理の頭脳として利用する
